...
parser.add_argument('--data',type=str,default='/home/hjl/deep_learning_workspace/dataset_cn/726/',help='data path')
...
parser.add_argument('--num_nodes',type=int,default=209,help='number of nodes/variables')
...
def ChangeEdgeIndexToAdjMatrix(edge_index, num_nodes):
    # edge_index: [2, E]s
    # edge_w: [E, dim_of_edge_features
    data = np.ones(shape=(edge_index.shape[1]))
    adj = sp.coo_matrix(arg1=(data,(edge_index[0, :], edge_index[1, :])),
                        shape=(num_nodes, num_nodes))
    adj = adj.todense()
    identity = torch.eye(num_nodes)
    adj = torch.from_numpy(adj) + identity
    return adj
...
def main(runid):
    ...
    dataloader = load_dataset_aqi(args.data, args.batch_size, args.batch_size, args.batch_size)
    scaler = dataloader['scaler']

    # 获取邻接矩阵
    # predefined_A = np.float32(np.load(os.path.join(args.data, 'edge_index.npy'), allow_pickle=True))
    # predefined_A = ChangeEdgeIndexToAdjMatrix(predefined_A, args.num_nodes)
    ...
    model = SandwichGNN(seq_len=args.seq_in_len, predefined_A=predefined_A, in_dim=args.in_dim
                        ).to(args.device)
