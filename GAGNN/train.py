...
from GAGNN.model import Model
from GAGNN.dataset_meta_la import trainDataset_metr_la, valDataset_metr_la, testDataset_metr_la
from util import *
...
for times in range(args.run_times):
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(test_loader):
            data = [item.to(device, non_blocking=True) for item in data]
            x, y, edge_index, edge_w, loc = data
            outputs = city_model(x, y, edge_index, edge_w, loc)
            cal_loss(outputs, y)
