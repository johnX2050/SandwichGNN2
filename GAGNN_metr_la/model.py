...
class Model(nn.Module):
    def __init__(self, mode, encoder, w_init, w, x_em, date_em, loc_em, edge_h, gnn_h, gnn_layer, city_num, group_num,
        ...
        if self.encoder == 'self':
            self.encoder_layer = TransformerEncoderLayer(2, nhead=2, dim_feedforward=256)
            # self.x_embed = Lin(8, x_em)
            self.x_embed = Lin(TIME_WINDOW * 2, x_em)

        elif self.encoder == 'lstm':
            self.input_LSTM = nn.LSTM(2, x_em, num_layers=1, batch_first=True)
        if self.w_init == 'rand':
        ...
        self.edge_inf = Seq(Lin(x_em * 2 + loc_em * 2, edge_h), ReLU(inplace=True))
    ...
    def forward(self, x, edge_index, edge_w, loc):
        ...
        g_edge_index = g_edge_index.repeat_interleave(g_x.shape[0], dim=0) # batch_size is not fixed!!!
